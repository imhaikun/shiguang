...
async function sendEmail(env: Env, to: string, code: string): Promise<boolean> {
  if (env.SMTP_HOST && env.SMTP_USER && env.SMTP_PASS) {
    return await sendEmailSMTP(env, to, code);
  }
  if (!env.MAIL_FROM) {
    return false;
  }
...
async function sendEmailSMTP(env: Env, to: string, code: string): Promise<boolean> {
  try {
    ...
    const reader = new Deno.BufferReader(socket);
    const writer = new Deno.BufferWriter(socket);
    ...
    await readLine();
    const tls = Deno.startTls(socket, { hostname: host });
    const tlsReader = new Deno.BufferReader(tls);
    const tlsWriter = new Deno.BufferWriter(tls);

    const tlsReadLine = async () => {
      const line = await tlsReader.readString("\r\n");
      return line?.trim() || "";
    };

    const tlsWriteLine = async (line: string) => {
      await tlsWriter.writeString(line + "\r\n");
      await tlsWriter.flush();
    };

    await tlsReadLine();
    await tlsWriteLine(`EHLO ${host}`);
    while (true) {
      const line = await tlsReadLine();
      if (line.startsWith("250 ")) break;
    }

    await tlsWriteLine("AUTH LOGIN");
    await tlsReadLine();
    await tlsWriteLine(btoa(user));
    await tlsReadLine();
    await tlsWriteLine(btoa(pass));
    await tlsReadLine();

    await tlsWriteLine(`MAIL FROM:<${from}>`);
    await tlsReadLine();
    await tlsWriteLine(`RCPT TO:<${to}>`);
    await tlsReadLine();
